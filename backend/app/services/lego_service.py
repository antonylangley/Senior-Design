...
from app.models.lego import (
    ...
    LegoDebugImages, LegoDetectResponse, NumericColor,
)
...
def _debug_images(frame: np.ndarray, mask: np.ndarray, bricks: list[LegoBrick]) -> LegoDebugImages:
...
def process_lego_image(image_data: str, options: LegoDetectionOptions) -> LegoDetectResponse:
    ...
    frame = resize_to_max_side(bytes_to_cv2(image_bytes), 1800)
    ...
    for index, instance in enumerate(instances, start=1):
        ...
        dimensions = infer_brick_dimensions(stud_centers, pose)
        ...
        confidence = min(0.95, 0.45 + 0.30 * fill + (0.20 if dimensions else 0.0))
        bricks.append(LegoBrick(
            ...
            confidence=round(confidence, 3), representative_color=NumericColor(hsv=hsv, lab=lab),
        ))
    ...
    return LegoDetectResponse(
        ...
        bricks=bricks,
        debug=_debug_images(frame, mask, bricks) if options.debug else None,
        warning=warning,
    )
